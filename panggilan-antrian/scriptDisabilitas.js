...
function insertData(){
    ...
    firebase.database().ref("antrian/layananDisabilitas").child(numInput.value).set({
            'timestamp' : Date.now(),
...
function getData(){
    firebase.database().ref("antrian").child("layananDisabilitas").on("value", (snap) => {
    ...
    //    Mengecek nilai total dari nilai status 0 1 dan 2
...
function selesaiAntrian(key, noAntrian){
    firebase.database().ref("antrian/layananDisabilitas").child(key).set({
        timestamp: Date.now(),
        noAntrian: noAntrian,
        status: 2,
        soundState: 0
...
function resetData(){
    firebase.database().ref("antrian/layananDisabilitas").remove()
}

function panggilAntrian(noAntrian, antrianKey, hitungNilaiSoundState){
    if(hitungNilaiSoundState < 1 ){ 
        firebase.database().ref("antrian/layananDisabilitas").child(antrianKey).set({
            timestamp: Date.now(),
            noAntrian: noAntrian,
            status: 1,
            soundState: 1,
