...
function insertData(){
    ...
    firebase.database().ref("antrian/layananKunjungan").child(numInput.value).set({
            'timestamp' : Date.now(),
...
function getData(){
    firebase.database().ref("antrian").child("layananKunjungan").on("value", (snap) => {
...
function selesaiAntrian(key, noAntrian){
    firebase.database().ref("antrian/layananKunjungan").child(key).set({
        timestamp: Date.now(),
        noAntrian: noAntrian,
        status: 2,
        soundState: 0
...
function setAntrianSekarang(){
    firebase.database().ref("antrian/layananKunjungan").orderByChild("timestamp").on("value", (snap) => {
...
function resetData(){
    firebase.database().ref("antrian/layananKunjungan").remove()
}
...
function panggilAntrian(noAntrian, antrianKey, hitungNilaiSoundState){
    ...
    console.log("2")
        ...
        firebase.database().ref("antrian/layananKunjungan").child(antrianKey).set({
            timestamp: Date.now(),
            noAntrian: noAntrian,
            status: 1,
            soundState: 1,
